...
class Backtester:
    ...
    # %%
    # calc_ret takes a two data frames (differing in size by one row) and returns an element wise multiplication.
    #   logic_df: dataframe of ones and zeros, ones representing when an algorithm is holding an asset, zeros when
    #   not holding the asset.
    #   ret: returns at each data point (except the last since N data points yields N-1 returns values)
    @staticmethod
